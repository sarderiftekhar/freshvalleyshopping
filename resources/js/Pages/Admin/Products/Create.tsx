...
export default function ProductCreate({ categories, brands }: Props) {
    ...
    const generateSku = useCallback((title: string) => {
        ...
        axios.post('/admin/products/generate-sku', { title })
    ...
    const handleTitleChange = (value: string) => {
        ...
        skuTimer.current = setTimeout(() => generateSku(value), 500);
    };
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                <div className="lg:col-span-2">
                                    ...
                                    <input type="text" value={data.title} onChange={(e) => handleTitleChange(e.target.value)} className={inputClass} />
                                    {errors.title && <p className={errorClass}>{errors.title}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}>SKU * <span className="text-xs text-gray-400 font-normal">(auto-generated)</span></label>
                                    <div className="relative">
                                        <input type="text" value={data.sku} onChange={(e) => setData('sku', e.target.value)} className={`${inputClass} pr-10`} />
                                        <button
                                            type="button"
                                            onClick={() => generateSku(data.title)}
                                    ...
                                    <div className="flex items-center gap-4">
                                        {barcodePreview && (
                                            <div className="relative group">
                                                <img src={barcodePreview} alt="Barcode" className="h-16 rounded-lg border border-gray-200 object-contain bg-white px-2" />
                                                <button
                                                    type="button"
                                                    onClick={() => { setBarcodePreview(null); setData('barcode_image', null as any); }}
                                        ...
                                        <label className="inline-flex items-center gap-2 px-3 py-2 border border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-orfarm-blue hover:text-orfarm-blue transition-colors">
                                            ...
                                            {barcodePreview ? 'Replace' : 'Upload Barcode'}
                                            <input
                                                type="file"
                                                accept="image/*"
                                                className="hidden"
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-4">
                                    {imagePreview.map((src, i) => (
                                        <div key={i} className="relative group">
                                            ...
                                            {i === 0 && <span className="absolute bottom-1 left-1 text-[10px] bg-orfarm-blue text-white px-1.5 py-0.5 rounded">Primary</span>}
                                        </div>
                                    ))}
                                ...
                                <div className="flex gap-2 mb-3 flex-wrap">
                                    {data.tags.map((tag) => (
                                        <span key={tag} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orfarm-blue text-white rounded-full text-sm font-medium shadow-sm hover:bg-orfarm-blue/80 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                                            {tag}
                                            <button type="button" onClick={() => removeTag(tag)} className="text-white/70 hover:text-white hover:bg-white/20 rounded-full p-0.5 transition-colors">
                                                <X className="size-3" />
                                            </button>
                                        </span>
                                    ))}
