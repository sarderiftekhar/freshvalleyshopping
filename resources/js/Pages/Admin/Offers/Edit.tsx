...
export default function OfferEdit({ offer, products }: Props) {
    ...
    const { data, setData, processing, errors } = useForm({
        ...
        buy_quantity: String(offer.buy_quantity || ''),
        get_quantity: String(offer.get_quantity || ''),
        minimum_order: offer.minimum_order || '',
        starts_at: formatDateForInput(offer.starts_at),
        ends_at: formatDateForInput(offer.ends_at),
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        router.post(`/admin/offers/${offer.id}`, formData, { forceFormData: true });
    };
    ...
    return (
        <AdminLayout title={`Edit: ${offer.title}`}>
            <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6">
                    ...
                    <input type="text" value={productSearch} onChange={(e) => setProductSearch(e.target.value)} placeholder="Search products..." className={`${inputClass} mb-3`} />
                    <div className="max-h-60 overflow-y-auto border border-gray-200 rounded-lg divide-y divide-gray-100">
                        {filteredProducts.slice(0, 50).map((product) => (
                            <label key={product.id} className="flex items-center gap-3 px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                <input type="checkbox" checked={data.product_ids.includes(product.id)} onChange={() => toggleProduct(product.id)} className="rounded border-gray-300" />
                ...
                <button type="submit" disabled={processing} className="px-6 py-3 bg-orfarm-blue text-white rounded-lg font-medium hover:bg-orfarm-blue/90 disabled:opacity-50">
